...
int main() {
  // WEDNESDAY: construct three objects and test the toString member function
  ...
  // FRIDAY: add duration objects together in several ways
  //   0. Very brief discussion of const to prohibit modifying objects
  ...
  //   5. Using + overloaded as nonmember nonfriend function
  //   6. Using + overloaded as friend function
  //   7. Using += overloaded as [students' choice!]

  //Duration total = d1.sumDuration(d2); // 1 member fn
  //Duration total = sumDuration(d1,d2); // 2-3 nonmember fn (friend or not)
  Duration total = d1+d2; // 4-6 overloaded + (member or not, friend or not)
