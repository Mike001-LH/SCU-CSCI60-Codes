// Interface and implementation of templatized linked list bag with iterator
...
template <typename T>
Bag<T>& Bag<T>::operator =(const Bag<T>& rhs) {//Bag<T>是类名，Bag<T>&是返回类型，Bag<T>::operator =是成员函数的定义
