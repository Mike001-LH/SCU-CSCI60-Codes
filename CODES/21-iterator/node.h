...
// Sara Krehbiel, 11/15/25-11/19/25
...
ostream& operator <<(ostream& o, const node<T>* curr) {
    ...
    */
	...
	if (curr == nullptr) return o;//如果curr为空，返回ostream对象，递归终止
    return o << curr->data() << " " << curr->link();//每次curr->data()和空格输出，然后递归调用curr->link()
}
...
void list_insert_tail(node<T> *& h, node<T> *& t, const T& val) {
    if (t==nullptr) { // check if this is the first node being inserted
		...
		t->set_link(new node<T>(val, nullptr)); // point old last node to new one//把老的最后一个节点指向新的节点
		t = t->link(); // increment the tail pointer//把tail指向新的最后一个节点
    }
...
void list_copy(node<T>*& lhead, node<T>*& ltail, const node<T>* rhead) { 
    // FRIDAY TODO: one more in-class recursion example
}
