...
public:
  ...
  iterator begin() const; // WEEK 10 TODO: write/call constructors accordingly
  iterator end() const; 
...
typename Bag<T>::iterator Bag<T>::begin() const {
	return L_iterator<T>(head); // return iterator pointing to first node
}

template <typename T>
typename Bag<T>::iterator Bag<T>::end() const {
	return L_iterator<T>(nullptr); // return iterator pointing past last node
}
